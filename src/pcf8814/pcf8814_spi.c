...
/*
Exp connector - signal - LCD Conn - 3510 Nokia LCD
15/20			3.3v		1		7
11				SCK			2		5
10				SDA			3		4
13				DC			4		nc
5				CS			5		2
1/2/8/19		GND			6		3
				Vout		7 		8				0.22uf to GND
6				Reset		8		1
							nc		6

^ pin 13 (BIT2/DCu) should be pulled to 3.3v via a resistor

*/

int initPCF8814_USB (TREGISTEREDDRIVERS *rd)
{
	TDISPLAYDRIVER dd;
	int dnumber;
	...
	l_strcpy(dd.name, "PCF8814:SPI");
	l_strcpy(dd.comment, "PCF8814:SPI. CS = EXP Pin 5");
	dnumber = registerDisplayDriver(rd, &dd);
	setDefaultDisplayOption(rd, dnumber, lOPT_PCF8814_CS, CS);
...
void closePCF8814_USB (TREGISTEREDDRIVERS *rd)
{
	return;
}
...
static void pcf8814_write_command (TDRIVER *drv, ubyte data)
{
	drv->pd->write16(drv->dd->port, data);
}
...
static void pcf8814_initiate (TDRIVER *drv)
{
	...
	pcf8814_write_command(drv, 0x1A);	/* p=automatically set */ 
...
int initPCF8814_USB(void *rd){return 1;}
void closePCF8814_USB(void *rd){return;}
