...
int ftdi_init (struct ftdi_context *ftdi)
{
    ...
    if ((ftdi->async_usb_buffer=l_malloc(sizeof(struct usbdevfs_urb)*ftdi->async_usb_buffer_size)) == NULL)
...
{
    struct ftdi_context * ftdi = (struct ftdi_context *)l_malloc(sizeof(struct ftdi_context));
...
void ftdi_deinit(struct ftdi_context *ftdi)
{
    if (ftdi->async_usb_buffer != NULL) {
        l_free(ftdi->async_usb_buffer);
        ftdi->async_usb_buffer = NULL;
    }

    if (ftdi->readbuffer != NULL) {
        l_free(ftdi->readbuffer);
        ftdi->readbuffer = NULL;
    }
}
    ...
    *curdev = NULL;
    for (bus = usb_busses; bus; bus = bus->next) {
        for (dev = bus->devices; dev; dev = dev->next) {
            ...
            {
                *curdev = (struct ftdi_device_list*)l_malloc(sizeof(struct ftdi_device_list));
                if (!*curdev)
...
int ftdi_usb_open_dev(struct ftdi_context *ftdi, struct usb_device *dev)
{
	...
	usb_set_configuration(ftdi->usb_dev, 1);
...
int ftdi_write_data(struct ftdi_context *ftdi, unsigned char *buf, unsigned int size)
{
    ...
    while (offset < size) {
        ...
        ret = usb_bulk_write(ftdi->usb_dev, ftdi->in_ep, (void*)(buf+offset), write_size, ftdi->usb_write_timeout);
        if (ret < 0)
...
{
    int ret = 1, i, num_of_chunks, chunk_remains;
    unsigned int offset = 0;
    ...
    while (offset < size && ret > 0) {
        ...
        ret = usb_bulk_read(ftdi->usb_dev, ftdi->out_ep, (void*)ftdi->readbuffer, ftdi->readbuffer_chunksize, ftdi->usb_read_timeout);
        if (ret < 0)
...
int ftdi_read_data_set_chunksize (struct ftdi_context *ftdi, unsigned int chunksize)
{
    ...
    ftdi->readbuffer_remaining = 0;
	if (ftdi->readbuffer == NULL)
		ftdi->readbuffer = l_malloc(4);

    if ((new_buf = (unsigned char *)l_realloc(ftdi->readbuffer, chunksize)) == NULL)
...
int ftdi_read_eeprom(struct ftdi_context *ftdi, unsigned char *eeprom)
{
    ...
    for (i = 0; i < ftdi->eeprom_size/2; i++) {
        if (usb_control_msg(ftdi->usb_dev, 0xC0, 0x90, 0, i, (void*)(eeprom+(i*2)), 2, ftdi->usb_read_timeout) != 2)
