...
// SED1565 (KS0723/S6B1713) display driver
...
// sed1565 parallel interface code submitted by Philipp Leusmann.
...
int initSED1565pio (TREGISTEREDDRIVERS *rd)
{
	TDISPLAYDRIVER dd;

	l_strcpy(dd.name, "SED1565:P");
	l_strcpy(dd.comment, "SED1565 display driver. Using parallel interface.");
...
void closeSED1565pio (TREGISTEREDDRIVERS *rd)
{
	return;
}
...
static int sed1565_RefreshArea (TDRIVER *drv, TFRAME *frm, int x1, int y1, int x2, int y2)
{
	...
	int ret =  sed1565_updateArea(drv, frm, x1, y1, x2, y2);
	drv->pd->flush(drv->pd);
	return ret;
}
...
static INLINE void sed1565_write_control (TDRIVER *drv, ubyte data)
{
	drv->pd->write8(drv->dd->port+2, data);
}

static INLINE void sed1565_write_data (TDRIVER *drv, ubyte data)
{	
	drv->pd->write8(drv->dd->port, data);
	sed1565_write_control(drv, 1);
	sed1565_write_control(drv, 0);
	sed1565_write_control(drv, 5);
} 

static INLINE void sed1565_write_command (TDRIVER *drv, ubyte data)
{
	drv->pd->write8(drv->dd->port, data);
	sed1565_write_control(drv, 9);
	sed1565_write_control(drv, 8);
	sed1565_write_control(drv, 13);
...
static void sed1565_initiate (TDRIVER *drv)
{
    ...
    */
    ...
    sed1565_write_control(drv, 15); //reset low
    sed1565_write_control(drv, 13);
    
    sed1565_write_command(drv, 0x40); //Start dislpay Line = 0
    sed1565_write_command(drv, 0xA0); //ADC = normal
    sed1565_write_command(drv, 0xA3); //LCD-Bias = 1/7
    sed1565_write_command(drv, 0xC0); //Output-mode unmirrored
    sed1565_write_command(drv, 0x2F); //Turn on LCD Voltage regulator
    sed1565_write_command(drv, 0x20); //Turn off 5V Regulator Resistor (Using external resistor)
    sed1565_write_command(drv, 0xAC); //static indicator off
    sed1565_write_command(drv, 0x81); //set contrast
    sed1565_write_command(drv, 0x20); //contrast = medium
    sed1565_write_command(drv, 0xA4); //turn off displaytest
    sed1565_write_command(drv, 0xAF); //turn on display 
    sed1565_write_command(drv, 0xA6); //display not inverted
...
int initSED1565pio(void *rd){return 1;}
void closeSED1565pio(void *rd){return;}
